...
[tout,~,te,~,ie] = ode15s(odefun,tspan,x0,options);
...
if mode.combustion_on % for hot fire only 
    [F_thrust, ~, M_e, p_exit, p_shock, m_cc_dot, M_cc_dot, ...
...
function [m_dot_lox, m_dot_gox, m_dot_oxtank_press, T_dot_drain, p_crit, m_dot_ox_crit] = ...
    N2OTankMDot(inputs, state, time)
% Calculates flow rate out of N2O Tank
...
%% N2OTankEquilibrium
function [m_dot_vap, T_dot_vap] = N2OTankEquilibrium(~, state)
% Find equilibrium state of nitrous oxide tank
%   Inputs:
...
function [F_thrust, OF, M_e, p_exit, p_shock, m_cc_dot, M_cc_dot, ...
    gamma_cc_dot, T_cc_dot] = CombustionChamber(inputs, state, m_dot_ox, m_dot_f)
% Model combustion chamber dynamics
%   Inputs:
%       state: object describing motor characteristics
%       state: object representing state of system
...
% Simulates dynamics of draining fuel tank
%   Inputs:
%       - inputs: object representing motor inputs
%       - state: object representing state of system
%       - time: time
...
%% CombustionCalc
function [gamma_ex, iMW, R_ex, iTc, c_star] = CombustionCalc(OF,Pc,inputs,CombData)
...
% Models flow of pressurant gas from source tank to
% propellant tank
%   Inputs:
